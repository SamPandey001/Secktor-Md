...
async function fetchRandomImages(query, nn) {
  const images = [];
  for (let i = 0; i < nn; i++) {
    try {
      let n = await gis(query);
      let randomImage = n[Math.floor(Math.random() * n.length)];
      images.push(randomImage.url);
    } catch (error) {
...
resolve(picsecktorh);
      ...
      let todlink = LangG.title + 'HD image 1920*1080'; 
      
      let imageURLs = await fetchRandomImages(todlink, 20);
      
      // Return a random image from the fetched images
      let picsecktorh = imageURLs[Math.floor(Math.random() * imageURLs.length)];
