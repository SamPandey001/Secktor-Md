			...
			try {
				...
				async function handleUserMessage(userId, userName, groupId) {
					try {
					  ...
					  user.xp = (user.xp || 0) + xpIncrement;
				  
					  const groupMessages = user.groups?.get(groupId) || 0;
					  user.groups = user.groups || new Map();
					  user.groups.set(groupId, groupMessages + 1);
				  
					  const nextLevelXp = 5 * Math.pow((user.level || 0) + 1, 2) + 50 * ((user.level || 0) + 1) + 100;
					  ...
					  // console.log(`🎉 Congrats ${userName}! You've leveled up to Level ${user.level}! 🎉`);
					} catch (error) {
