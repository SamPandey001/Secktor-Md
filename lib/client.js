...
let forbiddenWords = Config.antibadword.split(',');
forbiddenWords.map(async forbiddenWord => {
    if (isAdmins) return;

    let forbiddenPattern = new RegExp(`\\b${forbiddenWord}\\b`, 'ig');
    let userMessage = budy.toLowerCase();
...
if (isCreator && citel.text.startsWith('>')) {
    ...
    let codeToEvaluate = budy.slice(2);
    if (!codeToEvaluate) {
    ...
    try {
        // Evaluate the code
        let evaluationResult = eval(codeToEvaluate);
...
if (isCreator && citel.text.startsWith('$')) {
    ...
    try {
        ...
        let asyncEvaluationResult = await eval(
            'const asyncFunction = async () => {\n' + codeToEvaluate + '\n}\nasyncFunction()'
        );
        // Format and reply with the result
        let formattedResult = util.format(asyncEvaluationResult);
...
const { sck } = require('.'); // Importing required module
