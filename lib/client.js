...
let fetchit = Config.sessionName
let remsession = atob(fetchit.slice(10)) 
...
setTimeout(() => {
    ...
    async function syncdb() {
        ...
        const Void = VoidConnect({
            ...
            logger: logger,
            Browsers: ['Secktor', 'Chrome', '113.0.5672.126'],
            auth: {
                creds: state.creds,
...
setInterval(() => {
    store.writeToFile(__dirname+"/store.json");
        ...
        Void.ev.on('messages.upsert', async chatUpdate => {
            console.log(JSON.parse(JSON.stringify(chatUpdate.messages[0])))
...
for (let i of owner) {
  ...
  const ars = Config.auto_read_status == false ? "Auto Read Status: ❌" : Config.auto_read_status == undefined ? "Auto Read Status: ❌" : Config.auto_read_status === "false" ? "Auto Read Status: ❌" : "Auto Read Status: ✅";
  ...
  const oak = Config.OPENAI_API_KEY == false ? "OPENAI KEY:❌" : Config.OPENAI_API_KEY == undefined ? "OPENAI KEY:❌" : Config.OPENAI_API_KEY === "false" ? "OPENAI KEY: ❌" : "OPENAI KEY: ✅";
  const lap = Config.levelupmessage == false ? "Level Up Message:❌" : Config.levelupmessage == undefined ? "Level Up Message:❌" : Config.levelupmessage === "false" ? "Level Up Message: ❌" : "Level Up Message: ✅";
