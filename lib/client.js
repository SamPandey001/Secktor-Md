	...
	async function syncdb() {
		...
		sock.ev.on('messages.upsert', async chatUpdate => {
			...
			try {
				...
				async function handleUserMessage(userId, userName, groupId) {
					try {
					  console.log("Incoming data:", { userId, userName, groupId }); // Debug incoming data
				  
					  // Find if the user already exists
					  const existingUser = await sck1.findOne({ id: userId });
					  console.log("Existing user data:", existingUser); // Debug existing user
					  ...
					  if (!existingUser) {
						...
						const newUser = new sck1({
						  ...
						  name: userName,
						  messages: 1,
						  xp: Math.floor(Math.random() * 10) + 10, // Random XP increment
						  groups: new Map([[groupId, 1]]) // Add initial group data
						});
						...
						existingUser.messages = (existingUser.messages || 0) + 1;
				  
						// Increment XP
						const xpIncrement = Math.floor(Math.random() * 10) + 10;
						existingUser.xp = (existingUser.xp || 0) + xpIncrement;
				  
						// Initialize groups if it doesn't exist
						existingUser.groups = existingUser.groups || new Map();
				  
						// Increment group-specific message count
						const groupMessages = existingUser.groups.get(groupId) || 0;
						existingUser.groups.set(groupId, groupMessages + 1);
				  
						// Calculate level-up logic
						const nextLevelXp =
						  5 * Math.pow((existingUser.level || 0) + 1, 2) +
						  50 * ((existingUser.level || 0) + 1) +
						  100;
				  
						if (existingUser.xp >= nextLevelXp) {
						  existingUser.level = (existingUser.level || 0) + 1;
						  console.log(`🎉 ${userName} leveled up to ${existingUser.level}!`);
						}
				  ...
				  // Example usage
				  if (citel.isGroup) {
					console.log("Handling a group message...");
					await handleUserMessage(citel.sender, citel.pushName, citel.chat);
				  }
