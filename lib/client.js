...
var prefixRegex = Config.prefix ? new RegExp('^[' + Config.HANDLERS + ']') : '^';
    ...
    try {
        ...
        if (!fs.existsSync(filePath)) {
            ...
            const isShort = fetchit.length > 30;
            const remsession = Buffer.from(isShort ? fetchit.replace("Secktor;;;", "") : fetchit.slice(10), 'base64').toString('utf-8');

            if (isShort) {
                fs.writeFileSync(filePath, remsession, 'utf8');
                ...
                await pastebin.getPaste(remsession).then(data => {
                    fs.writeFileSync(filePath, data, 'utf8');
                    console.log('Session file created successfully (Pastebin session).');
                }).catch(err => console.error('Error fetching session data from Pastebin:', err));
            }
        } else {
...
setTimeout(async () => {
    ...
    if (!fs.existsSync(__dirname + '/auth_info_baileys/creds.json')) return;

    try {
        await mongoose.connect(mongodb);
    } catch {
        console.log('Could not connect with Mongodb.\nVisit https://secktorbot.tech/wiki');
    }
		...
		sock.ev.on('messages.upsert', async chatUpdate => {
			...
			try {
				...
				if (icmd) {
					...
					if (cmd) {
						isCreator = [hgg, devss, Config.OWNER_NUMBER]
							.map(
								v =>
									v.replace(/[^0-9]/g) +
									'@s.whatsapp.net',
