    ...
    fs.readdirSync(commandsDir).forEach((file) => {
        if (path.extname(file).toLowerCase() === '.js') {
            try {
                ...
                console.error(`Failed to require plugin ${file}:`, error);
            }
        }
    });
}
...
function watchCommandsDir() {
            ...
            if (eventType === 'rename') {
                fs.stat(filePath, (err, stats) => {
                    if (err && err.code === 'ENOENT') {
                        ...
                        plugindb.deleteOne({ id: path.basename(filename, '.js') }).catch(console.error);
                    }
                });
            }
        }
    ...
    console.log('Watching commands directory for changes...');
    requireAllCommands();
}