...
exports.smsg = (conn, citel, store) => {
    ...
    let quoted = (citel.quoted = 
      citel.msg && citel.msg.contextInfo && citel.msg.contextInfo.quotedMessage
    ...
    if (citel.msg && citel.msg.url) { citel.download = () => conn.downloadMediaMessage(citel.msg);}  
  
    citel.text = citel.msg?.text 
    || citel.message?.imageMessage?.caption 
    || citel.message?.videoMessage?.caption 
    || citel.message?.conversation 
    || citel.msg?.contentText 
    || citel.msg?.selectedDisplayText 
    || citel.msg?.title 
    || '';
	...
	*/
	...
	citel.replyimg = (img, teks, id = citel.chat, option = { mentions: [citel.sender] }) => conn.sendMessage(id, { image: img, caption: teks, contextInfo: { mentionedJid: option.mentions } }, { quoted: citel })
        ...
        citel.reply = async (content, opt = { packname: "Secktor", author: "SamPandey001" }, type = "text") => {
          const send = (type, data) => conn.sendMessage(citel.chat, { [type]: data, ...opt }, { quoted: citel });
          ...
          switch (type.toLowerCase()) {
              ...
              case "video":
              case "audio":
                  const media = Buffer.isBuffer(content) ? content : isUrl(content) ? { url: content } : null;
                  if (media) return send(type, media);
                  break;
      
              case "template":
                  const optional = await generateWAMessage(citel.chat, content, opt);
                  const message = { viewOnceMessage: { message: { ...optional.message } } };
                  return conn.relayMessage(citel.chat, message, { messageId: optional.key.id });
      
              case "sticker":
                  const { data, mime } = await conn.getFile(content);
                  if (mime === "image/webp") {
                      const buff = await writeExifWebp(data, opt);
                      return send("sticker", { url: buff });
                  }
                  const mimeType = mime.split("/")[0];
                  if (["image", "video"].includes(mimeType)) {
                      return await conn.sendImageAsSticker(citel.chat, content, opt);
                  }
                  break;
