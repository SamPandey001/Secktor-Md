...
cmd({
  ...
  let __lobQ = text.split("/addstickers/")[1];
  let {
    result: __loby
  } = await fetchJson("https://api.telegram.org/bot1891437832:AAFir-uJY5hR53FWbciV9k9getStickerSet?name=" + encodeURIComponent(__lobQ));
  if (__loby.is_animated) {
    return citel.reply("_Animated stickers are not supported_");
  }
  citel.reply(("*Total stickers :* " + __loby.stickers.length + "\n*Estimated complete in:* " + __loby.stickers.length * 1.5 + " seconds").trim());
  for (let __lobr of __loby.stickers) {
    let __lobK = await fetchJson("https://api.telegram.org/bot1891437832:AAFir-uJY5hR53FWbciV9k9/getFile?file_id=" + __lobr.file_id);
    let __lobb = await getBuffer("https://api.telegram.org/file/bot1891437832:AAFir-uJY5hR53FWbciV9/" + __lobK.result.file_path);
    await Void.sendImageAsSticker(citel.chat, __lobb, citel, {
